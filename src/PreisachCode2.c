...
//for use with PreisachModelTutorial
...
void initializeGlobals(int camXRes, int camYRes, int maxBlocks, double refractoryTime, double probDown){
	FILE *fp;
	int offsetARRAY[4];
	double sizeARRAY[2];
...
int Preisach_Main(int length, uint8 * input, double scalarInput){
	...
	//input is the camera yuv input (only y values)
	//scalarInput is user entered PM scalar input value
	int i;
	FILE * fout;
	...
	globals.model.input1=scalarInput;//input1 is new input
	...
	setvoltage(calculate_voltage_input(OUTARRAY[2]),calculate_voltage_output(OUTARRAY[1]));
	//output to screen
